...
- `--no-approval` 플래그 사용 시 최종 APPROVE 후 Plan Mode 없이 요약만 출력하고 종료
- `--no-approval` 미지정 시 (기본값): 최종 APPROVE 후 Plan Mode 진입하여 사용자 승인 획득
</Execution_Policy>
...
- **기술 스택 선정**: 서버 프레임워크는 4.1절 선정 기준(보안 요건 → 특화 요건 → 생태계)을 따름. 네이티브/시스템 컴포넌트는 플랫폼 제약을 따름
- **모듈 승격 판정**: 기능 항목을 독립 모듈로 분리하기 전 2.1.2절 기준(독립 빌드 산출물, 외부 인터페이스, 공유 라이브러리)을 충족하는지 확인. 미충족 시 상위 모듈의 FR로 통합
...
6. **Design Guide Compliance** (설계 가이드 존재 시): 설계가 system-design-guide.md 원칙을 준수하는가?
   - Shell/Core 분리, 인터페이스 유형별 정의 항목, 프로토콜 소유권, 요구사항 ID 체계, 기술 스택 선정 기준
   - **모듈 승격 판정**: 각 모듈이 2.1.2절 승격 기준(독립 빌드 산출물 / 외부 인터페이스 / 공유 라이브러리) 중 하나 이상을 충족하는가? 내부 서비스를 과도하게 독립 모듈로 분리하지 않았는가?
...
```
APPROVE:
  → `--no-approval` 미지정 시: Plan Mode 진입 → 사용자에게 설계 + 계획 요약 표시 → 사용자 승인/수정/거부 선택
  → `--no-approval` 지정 시: 설계 + 계획 요약을 텍스트로 출력하고 종료 (Plan Mode 생략)
...
- **EnterPlanMode**: 최종 APPROVE 후 사용자 승인 흐름 진입 (`--no-approval` 미지정 시)
</Tool_Usage>
...
- [ ] 최대 5회 반복 제한이 적용되는가?
- [ ] 최종 APPROVE 후 사용자 승인을 받았는가? (`--no-approval` 시 요약 출력으로 대체)
