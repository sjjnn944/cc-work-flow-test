...
### Step 2.3.1: 테스트 프레임워크 설치

플랫폼별 빌드 도구 설치 완료 후, 테스트 프레임워크를 설치한다.

| 플랫폼 | 프레임워크 | 설치 방법 | 설치 조건 |
|--------|-----------|----------|----------|
| C++ | GTest | vcpkg install (`thirdparty/vcpkg/vcpkg install gtest`) | `test/` 디렉토리 존재 시 |
| Spring Boot | JUnit 5 | (설치 불필요 — `spring-boot-starter-test` gradle 의존성) | — |
| React | Vitest/Jest | (설치 불필요 — npm devDependencies) | — |
| Go | go test | (내장) | — |
| Rust | cargo test | (내장) | — |
| Django | pytest | (설치 불필요 — poetry/pip 의존성) | — |
| NestJS | Jest | (설치 불필요 — 프로젝트 scaffolding에 포함) | — |
| .NET | xUnit/NUnit | (설치 불필요 — NuGet 의존성) | — |

**C++ GTest 설치 흐름:**
1. `test/` 디렉토리가 존재하는지 확인
2. vcpkg가 설치된 상태에서 `vcpkg install gtest` 실행 (triplet은 OS에 맞게 자동 선택)
3. vcpkg manifest mode 사용 시: Step 3.1.1에서 vcpkg.json에 `gtest` 포함 → cmake configure 시 자동 설치

### Step 2.3.2: 정적 분석 도구 설치
...
- `dependencies` (자동 추가) — `test/` 디렉토리 존재 시 `gtest` 자동 포함
...
## Step 6: 인프라 명세 생성 (infrastructure-spec.md)

설계서 기술스택 절에서 인프라 서비스 정보를 추출하여 `doc/architecture/infrastructure-spec.md`를 생성한다.
`--install-only` 또는 `--build-only` 시 건너뛴다.

### 6.1 설계서 인프라 테이블 추출

`doc/designs/*.md`에서 Section 2 "기술 스택" / "인프라 서비스" 테이블을 Grep/Read로 추출한다.

### 6.2 container-registry.json 대조

각 서비스를 `.claude/skills/docker-infra/references/container-registry.json`과 대조한다:
- 등록됨 → 이미지, 호스트포트, 카테고리를 registry에서 가져옴
- 미등록 → "미등록" 표시 (docker-infra 자동등록 대상)

### 6.3 infrastructure-spec.md 생성

`doc/architecture/infrastructure-spec.md`를 생성한다:
- 카테고리별 서비스 목록
- 호스트 포트 (registry 기반)
- 의존성 순서
- compose 스택 매핑

이미 존재하면 덮어쓰지 않고 건너뛴다 (기존 파일 보호 정책 동일).

### 6.4 생성 결과 요약 출력

```
=== 인프라 명세 생성 ===
서비스: {N}개 (등록됨 {R}개 / 미등록 {U}개)
파일: doc/architecture/infrastructure-spec.md
```

## Step 7: 개발 도구 설정 파일 생성
