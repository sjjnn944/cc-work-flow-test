...
3. 기존 보고서 파일(`--report-path` 대상)이 있으면 자동으로 덮어쓴다 (별도 확인 없이 진행)
...
**조건부 승인:**
- 문서 누락 경고가 **0개** → 요약 출력 후 자동으로 Step 2 진행
- 문서 누락 경고가 **1개 이상** → 누락 목록을 표시하고 AskUserQuestion으로 진행 여부 확인
...
5. API 반환값 미검증 — 반환값을 변수에 저장했으나 검증하지 않는 코드
6. 리소스 누수 — 핸들(Event, Mutex, File), 메모리의 모든 경로 해제 여부
7. 잠금 범위 밖 포인터 역참조 — lock() → ptr = find() → unlock() → use(ptr)
8. 에러 경로 로그 누락 — if(FAILED) 또는 catch 블록에서 로그 없이 반환
...
3. verification-guide.md 3절 체크리스트 + 아래 추가 항목을 적용한다:
   - [ ] 모든 외부 호출의 반환값이 검증되는가 (저장 + 검증 모두)
   - [ ] 에러 경로마다 로그가 출력되는가 (로그 없는 에러 반환 금지)
   - [ ] 핸들/리소스가 모든 경로에서 해제되는가 (조기 반환 포함)
   - [ ] 잠금 보호 데이터의 포인터가 잠금 범위 안에서만 사용되는가
...
- **배치 내 1건 이상 워커 실패**: 실패 목록 + 사유를 표시하고 다음 배치 진행 여부 사용자 확인
</Escalation_And_Stop_Conditions>
